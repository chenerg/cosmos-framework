...
- **W&B defaults to online**: schema `[job].wandb_mode = "online"`. Export `WANDB_API_KEY` to log to W&B; any failure to use online (network, auth, 403, …) falls back to `"offline"`. Set `"disabled"` to skip W&B. Some vision recipes still pin `"disabled"`.
