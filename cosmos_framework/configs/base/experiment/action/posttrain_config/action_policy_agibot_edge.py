...
"""``action_policy_agibot_edge`` — Cosmos3-Edge AgiBot action-policy SFT recipe.

Edge-tier sibling of ``action_policy_droid_edge``: same Edge model baseline
(``EDGE_MODEL_CONFIG``, Nemotron-2B-Dense-VL backbone), optimizer / scheduler /
trainer / checkpoint blocks, and the same whole-episode + pre-TF token packing
path, but feeds ``AgiBotLeRobotDataset`` (dual-arm 29D ``ee_pose`` +
``use_state``, raw/un-normalized, concat_view: head cam on top, two wrist
cameras half-size below) through ``get_action_agibot_sft_dataset``.
``AGIBOT_ROOT`` is a LeRobot v3 dir (``meta/info.json``) or an ``Agibotworld/``
parent of ``task_*/canonical_55d`` shards.
"""
...
from cosmos_framework.data.generator.action.datasets.action_sft_dataset import get_action_agibot_sft_dataset
...
def _action_policy_agibot_edge_model_config() -> dict:
    """AgiBot model config on the Edge baseline: selective activation
...
action_policy_agibot_edge = LazyDict(
    dict(
        defaults=[
        ...
        job=dict(
            project="cosmos3",
            group="action_sft",
            name="action_policy_agibot_edge",
            wandb_mode="online",
        ),
        model=dict(
            config=_action_policy_agibot_edge_model_config(),
        ),
        optimizer=dict(
            betas=[0.9, 0.99],
            eps=1.0e-08,
            ...
            lr=2.0e-04,  # DROID nano / Edge reference (gbs 8192); local TOML may lower this
            lr_multipliers={
                "action2llm": 5.0,
                "llm2action": 5.0,
        ...
        scheduler=dict(
            ...
            f_max=[0.4],  # matches the DROID Edge recipe
            f_min=[0.0],
            f_start=[0.0],
        ...
        dataloader_train=L(PackingDataLoader)(
            ...
            # max_pre_tf_tokens=50000 (~3297 frames at 256p concat K=56
            # after letterbox crop, extra=500). lookahead=10: skip more too-large next
            # episodes to fill leftover packing budget.
            ...
            dataloader=L(RankPartitionedDataLoader)(
                batch_size=1,
                in_order=False,
                # Same 8-card TF worker layout as DROID Edge.
                num_workers=8,
                persistent_workers=True,
                pin_memory=True,
                prefetch_factor=2,  # 256p no-pack default. 480p: pf=1.
                sampler=None,
                ...
                datasets=dict(
                    agibot=dict(
                        ratio=1,
                        dataset=L(get_action_agibot_sft_dataset)(
                            # LeRobot v3 dir or Agibotworld/ parent of
                            # task_*/canonical_55d. Example:
                            #   AGIBOT_ROOT=.../Agibotworld/task_352/canonical_55d
                            root="${oc.env:AGIBOT_ROOT}",
                            fps=15.0,
                            ...
                            # length would exceed the packing token budget.
                            # Same helper as DROID: max_pre_tf_tokens + concat K.
                            # Index-only; workers never decode the dropped videos.
                            max_pre_tf_tokens=50000,
                            ...
                            viewpoint="concat_view",  # head (top) + L/R wrist 1/2 (bottom) → 256p 1:1
                            resolution="256",  # 720x640 concat @ 256p (256x256)
...
for _item in [action_policy_agibot_edge]:
