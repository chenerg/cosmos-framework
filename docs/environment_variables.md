...
- `WANDB_API_KEY`: Required when `job.wandb_mode = "online"` (the schema default). If online cannot be used, training falls back to offline logging. Set `wandb_mode = "disabled"` to skip W&B.
