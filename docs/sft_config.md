...
| `wandb_mode`                | `"online"`   | `"online"` (real-time, needs `WANDB_API_KEY`; any failure to use online falls back to `"offline"`), `"offline"` (log locally, sync later via `wandb sync`), or `"disabled"`.                                                                                                                                                                                                                   |
...
| `joint_attn_implementation`    | `"two_way"`                                                     | VFM attention layout: `"two_way"` (U/G blocks with cross-attention), `"three_way"` (adds sparsity-aware third block — NATTEN), or `"teacher_forcing"` (causal CLEAN/NOISY streams in training, two-way at inference). **VFM only.**                                                                               |
